...
export const LABELED_METRICS = new Set([
	'file_io_ops_total',
	'file_io_bytes_total',
 ...
 * into a compact key like `op=read` (single label) or `op=read,foo=bar` (multi-label)
 */
export function buildLabelKey(labelStr: string): string {
...
/**
 * Parses Prometheus text-exposition-format output into the flat metrics
 * shape the frontend consumes — scalar metrics as plain numbers, and
 * metrics listed in LABELED_METRICS collapsed into Record<label, value>.
 * Extracted from the /api/metrics route handler (issue #240) so the
 * parsing logic is unit-testable independent of the fetch/SvelteKit layer.
 */
export function parsePrometheusMetrics(text: string): Partial<AppMetrics> {
