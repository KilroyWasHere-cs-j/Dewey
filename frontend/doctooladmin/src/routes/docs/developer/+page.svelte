					...
					<table class="w-full text-sm">
						...
						<tbody class="divide-y divide-gray-100 dark:divide-gray-700">
							{#each [
								...
								{ method: 'POST',   path: '/files/move',                              handler: 'moveFile',                desc: 'Move/rename a stored file and update its DB filepath (issue #333). Takes {"current_path", "new_path"} as a JSON body rather than route params (issue #416) — both are store-relative paths, which almost always contain a subfolder once a file has gone through OnFilter\'s routing, and gin :param segments can\'t carry a "/".' },
