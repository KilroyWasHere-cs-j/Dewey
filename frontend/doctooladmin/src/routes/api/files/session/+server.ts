...
// Exchanges the files password for a session token (issue #409) — the
// only /api/files/* route that still forwards X-Dewey-Password rather
// than X-Dewey-Session-Token.
export const POST: RequestHandler = async ({ request }) => {
	try {
		const res = await fetch(`${backendUrl()}/core/files/session`, {
			method: 'POST',
